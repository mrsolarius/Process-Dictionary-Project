...
PAskFrame decodeAskFrame(char *frame){
    ...
    unsigned int tailleFrame = sizeof(frame);
    ...
    if(tailleFrame==4){
        switch (frame[0]){
            ...
            case A_DUMP:
                ...
                break;
            case A_LOOKUP:
                //Un lookup ne peut pas être un success sans data donc avec une taille fixe
                if(frame[2]==SUCCESS){
        ...
        acquittalFrame->dataLength=NULL;
        acquittalFrame->data=NULL;
    }else if (tailleFrame > 5){
        if(frame[0]!=A_LOOKUP){
            DDP_Errno = ENOTDDP;
        }
...
char *encodeAskFrame(PAskFrame askFrame){
    ...
    if(evaluateFrame(askFrame->cmd) != AskFrame){
        DDP_Errno=ENOTASK;
        exit(-1)
    }
    ...
    switch(length){
        ...
        case 4:
            frame[0]=askFrame->cmd;
            if(askFrame->val==NULL){
                DDP_perror = -1;//@todo il manque une erreur;
                exit(-1);
            }
